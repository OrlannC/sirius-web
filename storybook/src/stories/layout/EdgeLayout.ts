...
import { type InternalNode, type Node, Position, type XYPosition } from '@xyflow/react';
import { type Handle, type NodeLookup } from '@xyflow/system';
...
import { BorderNodePosition, getPositionAbsoluteFromNodeChange, isDescendantOf, isSiblingOrDescendantOf, type ConnectionHandle, type NodeData } from '@eclipse-sirius/sirius-components-diagrams';
import { horizontalLayoutAngleMax, horizontalLayoutAngleMin, verticalLayoutAngleMax, verticalLayoutAngleMin } from '@eclipse-sirius/sirius-components-diagrams/renderer/layout/layoutParams';
...
const getSegmentDirection = (edgePath: string, xyPosition: XYPosition): SegmentDirection => {
  const pathPoints = parseSVG(edgePath);
  ...
  for (let i = 1; i < pathPoints.length; i++) {
    ...
    if (p1 && p2) {
      const p1Y = (p1 as any).y ?? 0;
      const p2Y = (p2 as any).y ?? 0;
      const p1X = (p1 as any).x ?? 0;
      const p2X = (p2 as any).x ?? 0;
      const isVertical: boolean = p1Y === p2Y;
      const isHorizontal: boolean = p1X === p2X;
      const isOnVerticalSegment = isHorizontal && Math.round(x) === Math.round(p1X);
      const isOnHorizontalSegment = isVertical && Math.round(y) === Math.round(p1Y);
...
const getHandlePosition = (nodePositionChange: any, nodeA:any, nodeB:any, nodeLookup:any, layoutDirection:any): Position => {

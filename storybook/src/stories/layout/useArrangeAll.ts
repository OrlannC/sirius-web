...
import ELK, { type ElkLabel, type ElkNode } from 'elkjs/lib/elk.bundled';
import { useLayout, type DiagramNodeType, type EdgeData, type NodeData } from '@eclipse-sirius/sirius-components-diagrams';
...
import type { UseArrangeAllValue } from './useArrangeAll.types';
...
const computeLabels = (
  node: { data: { insideLabel: { id: any; text: any; }; outsideLabels: { BOTTOM_MIDDLE: { id: any; text: any; }; }; }; height: any; },
  viewportZoom: number,
  reactFlowWrapper: React.MutableRefObject<HTMLDivElement | null>
): ElkLabel[] => {
...
export const useArrangeAll = (reactFlowWrapper: React.MutableRefObject<HTMLDivElement | null>): UseArrangeAllValue => {
  ...
  const getELKLayout = async (
    ...
    const graph: ElkNode = {
      ...
      edges: edges.filter(
        (edge: { source: any; target: any; }) => nodes.some((node: { id: any; }) => node.id === edge.source) && nodes.some((node: { id: any; }) => node.id === edge.target)
      ),
    };
    try {
      ...
      return {
        nodes:
          layoutedGraph?.children?.map((node) => {
            const originalNode = nodes.find((node_1: { id: string; }) => node_1.id === node.id);

...
import {
  defaultAllAlgorithmsConfiguration,
  DiagramStoryArgs,
  getElkAllAlgorithmsOptions,
} from '../../utils/layout/layoutConfigurations';
import { getScenariosMap } from '../../utils/layout/scenario';

const meta: Meta<DiagramStoryArgs> = {
  ...
  argTypes: {
    ...
    directionL: {
      ...
      control: 'select',
      if: { arg: 'algorithm', eq: 'layered' },
      options: ['DOWN', 'RIGHT', 'LEFT', 'UP'],
    },
    ...
    edgeNodeBetweenLayers: {
      name: 'Edge spacing between layers',
      control: 'number',
      if: { arg: 'algorithm', eq: 'layered' },
    },
    layStrat: {
      ...
      control: 'select',
      if: { arg: 'algorithm', eq: 'layered' },
      options: ['NETWORK_SIMPLEX', 'LONGEST_PATH', 'LONGEST_PATH_SOURCE'],
    },
    nodePlacStrat: {
      name: 'Node placement strategy',
      control: 'select',
      if: { arg: 'algorithm', eq: 'layered' },
      options: ['NETWORK_SIMPLEX', 'LINEAR_SEGMENTS', 'SIMPLE', 'BRANDES_KOEPF'],
    },
    contentAlignLayered: {
    ...
    edgeRoutingMode: {
      ...
      control: 'select',
      if: { arg: 'algorithm', eq: 'mrtree' },
      options: ['AVOID_OVERLAP', 'MIDDLE_TO_MIDDLE', 'NONE'],
    },
    wheighting: {
      name: 'Weighting method',
      control: 'select',
      if: { arg: 'algorithm', eq: 'mrtree' },
      options: ['MODEL_ORDER', 'DESCENDANTS', 'FAN', 'CONSTRAINT'],
    },

    whiteSpaceStart: {
      name: 'White space distribution',
      control: 'select',
      if: { arg: 'algorithm', eq: 'rectpacking' },
      options: ['EQUAL_BETWEEN_STRUCTURES', 'TO_ASPECT_RATIO', 'NONE'],
    },
    approStrat: {
      name: 'Packing strategy',
      control: 'select',
      if: { arg: 'algorithm', eq: 'rectpacking' },
      options: ['GREEDY', 'TARGET_WIDTH'],
    },
    ...
    approOptiGoal: {
      ...
      control: 'select',
      if: { arg: 'algorithm', eq: 'rectpacking' },
      options: ['ASPECT_RATIO_DRIVEN', 'MAX_SCALE_DRIVEN', 'AREA_DRIVEN'],
    },

    wedgeCrit: {
      name: 'Wedge criterion',
      control: 'select',
      if: { arg: 'algorithm', eq: 'radial' },
      options: ['LEAF_NUMBER', 'NODE_SIZE'],
    },
    compactor: {
      name: 'Compaction method',
      control: 'select',
      if: { arg: 'algorithm', eq: 'radial' },
      options: ['NONE', 'RADIAL_COMPACTION', 'WEDGE_COMPACTION'],
    },
    optiCrit: {
      name: 'Optimization criterion',
      control: 'select',
      if: { arg: 'algorithm', eq: 'radial' },
      options: ['NONE', 'EDGE_LENGTH', 'EDGE_LENGTH_BY_POSITION ', 'CROSSING_MINIMIZATION_BY_POSITION'],
    },

    spanningTree: {
      name: 'Spanning tree metric',
      control: 'select',
      if: { arg: 'algorithm', eq: 'sporeCompaction' },
      options: [
        'CENTER_DISTANCE',
        'CIRCLE_UNDERLAP',
        'RECTANGLE_UNDERLAP',
    ...
    treeConstrution: {
      ...
      control: 'select',
      if: { arg: 'algorithm', eq: 'sporeCompaction' },
      options: ['MINIMUM_SPANNING_TREE', 'MAXIMUM_SPANNING_TREE'],
    },

    model: {
      name: 'Force model',
      control: 'select',
      if: { arg: 'algorithm', eq: 'force' },
      options: ['EADES', 'FRUCHTERMAN_REINGOLD'],
    },
...
const scenariosDict = getScenariosMap();
const listDiagram = Object.values(scenariosDict);
...
export const Basic: StoryObj<DiagramStoryArgs> = {
  ...
  args: defaultAllAlgorithmsConfiguration,
  render: (args) => {
    const mySpecificDiagram = scenariosDict['BasiqueSiriusDiagram'];
    return (
      <div style={{ width: '100vw', height: '100vh' }}>
        <DiagramStoryWrapper args={args} diagram={mySpecificDiagram} layoutOptions={getElkAllAlgorithmsOptions(args)} />
      </div>
    );
  },
};
...
export const FiveNodes: StoryObj<DiagramStoryArgs> = {
  ...
  args: defaultAllAlgorithmsConfiguration,
  render: (args) => {
    const mySpecificDiagram = scenariosDict['FiveNodeDiagram'];
    return (
      <div style={{ width: '100vw', height: '100vh' }}>
        <DiagramStoryWrapper args={args} diagram={mySpecificDiagram} layoutOptions={getElkAllAlgorithmsOptions(args)} />
      </div>
...
export const RobotFlow: StoryObj<DiagramStoryArgs> = {
  name: 'Scenario robot flow',
  args: defaultAllAlgorithmsConfiguration,
  render: (args) => {
    const mySpecificDiagram = scenariosDict['RobotFlow'];
    return (
      <div style={{ width: '100vw', height: '100vh' }}>
        <DiagramStoryWrapper args={args} diagram={mySpecificDiagram} layoutOptions={getElkAllAlgorithmsOptions(args)} />
      </div>
...
export const BorderNode: StoryObj<DiagramStoryArgs> = {
  name: 'Scenario with border node',
  args: defaultAllAlgorithmsConfiguration,
  render: (args) => {
    const mySpecificDiagram = scenariosDict['BorderNode'];
    return (
      <div style={{ width: '100vw', height: '100vh' }}>
        <DiagramStoryWrapper args={args} diagram={mySpecificDiagram} layoutOptions={getElkAllAlgorithmsOptions(args)} />
      </div>

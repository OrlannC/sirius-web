...
const ToolbarActions = () => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center' }}>
      <Tooltip title="Evaluate the diagram">
      ...
      <Tooltip title="Evaluate with AI">
        <span>
          {' '}
          <IconButton onClick={context.handleAiSubmit} disabled={context.isAILoading} size="small">
...
export const DiagramStoryWrapper = ({
  ...
  const client = useClient(initialDiagram as EnrichedGQLDiagram);
  ...
  const handleReadMetrics = () => {
    ...
    logScore({
      'Number of edge crossings': metrics.edgeCrossings,
      'Number of nodes crossed by different edges': metrics.edgeNodeCrossings,
      'Number of overlapping nodes': metrics.nodeOverlap,
      'Average number of bending points': metrics.averageBendingsPoints,
      'Space utilization (%)': metrics.spaceUtilization,
      'Edge length standard deviation': metrics.edgeStandardDeviation,
      'Flow orientation (%)': metrics.flowOrientation,
      'Spatial grouping (%)': metrics.spatialGrouping,
  ...
  const handleAiSubmit = async () => {
    ...
    try {
      ...
      if (!base64Image) {
        action('AI Error')('Unable to capture the diagram.');
        return;
      }
      ...
      const apiResponse = await fetch('http://localhost:8081/api/evaluate-diagram', {
        method: 'POST',
        headers: {
      ...
      action('AI Response')(result.answer);
    } catch (error) {
      console.error(error);
      action('AI Error')('Failed to send to AI. Check the console.');
    } finally {
        ...
        <ServerContext.Provider value={{ httpOrigin: '' }}>
          <SelectionContextProvider initialSelection={{ entries: [] }}>
            <ExtensionProvider registry={storybookRegistry}>
              <EvaluatorContext.Provider value={{ handleReadMetrics, handleAiSubmit, isAILoading }}>
                <div
                  ref={containerRef}
                  style={{
                    position: 'relative',

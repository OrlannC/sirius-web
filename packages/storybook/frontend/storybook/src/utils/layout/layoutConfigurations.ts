...
export type DiagramStoryArgs = ComponentProps<typeof DiagramRepresentation> & StoryCustomArgs;

export const getElkForceOptions = (args: Partial<DiagramStoryArgs>): LayoutOptions => {
...
export const getElkLayeredOptions = (args: Partial<DiagramStoryArgs>): LayoutOptions => {
  const def = defaultLayeredAlgorithmConfiguration;
  return {
    'elk.algorithm': 'layered',
    'elk.layered.spacing.nodeNodeBetweenLayers': String(args.nodeNodeBetweenLayers ?? def.nodeNodeBetweenLayers),
    'elk.layered.spacing.edgeNodeBetweenLayers': String(args.edgeNodeBetweenLayers ?? def.edgeNodeBetweenLayers),
    'elk.spacing.componentComponent': String(args.componentComponent ?? def.componentComponent),
    'elk.spacing.nodeNode': String(args.nodeNode ?? def.nodeNode),
    'elk.direction': args.direction ?? def.direction,
    'elk.layering.strategy': args.layStrat ?? def.layStrat,
    'elk.layered.nodePlacement.strategy': args.nodePlacStrat ?? def.nodePlacStrat,
...
export const getElkMrTreeOptions = (args: Partial<DiagramStoryArgs>): LayoutOptions => {
...
export const getElkRadialOptions = (args: Partial<DiagramStoryArgs>): LayoutOptions => {
...
export const getElkRectPackingOptions = (args: Partial<DiagramStoryArgs>): LayoutOptions => {
  const def = defaultRectPackingAlgorithmConfiguration;
  return {
    'elk.algorithm': 'rectpacking',
    'elk.spacing.nodeNode': String(args.nodeNode ?? def.nodeNode),
    'elk.rectpacking.trybox': 'true',
    'elk.rectpacking.whiteSpaceElimination.strategy': args.whiteSpaceStart ?? def.whiteSpaceStart,
    'elk.rectpacking.widthApproximation.strategy': args.approStrat ?? def.approStrat,
    'elk.rectpacking.widthApproximation.targetWidth': String(args.targetWidth ?? def.targetWidth),
    'elk.contentAlignment': args.contentAlign ?? def.contentAlign,
    'elk.rectpacking.widthApproximation.optimizationGoal': args.approOptiGoal ?? def.approOptiGoal,
  };
};
...
export const getElkSporeCompactionOptions = (args: Partial<DiagramStoryArgs>): LayoutOptions => {
  const def = defaultSporeCompactionAlgorithmConfiguration;
  return {
    'elk.algorithm': 'sporeCompaction',
    'elk.spacing.nodeNode': String(args.nodeNode ?? def.nodeNode),
    'elk.processingOrder.spanningTreeCostFunction': args.spanningTree ?? def.spanningTree,
    'elk.processingOrder.treeConstruction': args.treeConstrution ?? def.treeConstrution,
  };
};
...
export const getElkAllAlgorithmsOptions = (args: Partial<DiagramStoryArgs>): LayoutOptions => {
...
export const defaultAllAlgorithmsConfiguration = {
  ...
  algorithm: 'layered',
  aiPrompt:
    "Tu es un ingénieur senior en visualisation de données. Analyse cet agencement technique de manière pragmatique. CONSIGNES : 1. Focus Structurel : Ignore totalement l'esthétique (couleurs, polices, style). Concentre-toi sur le routage des arêtes et la disposition des blocs. 2. Échelle de Sévérité Contextuelle : - Petit graphe : Si la lecture est immédiate et qu'il n'y a pas d'erreur majeure (texte écrasé, flux illogique), la note doit être élevée. Ne pénalise pas les micro-décalages de pixels ou les coudes inutiles. - Gros graphe : Sois exigeant sur la gestion du chaos, les croisements et la compacité. 3. Note sur 10 : Évalue si le diagramme remplit sa mission d'information sans effort cognitif. Réponse courte : [Critique] | [Note]/10",
  nodeNode: 80,
